...
//#define BackgroundInterval 86400
#define BackgroundInterval 180-2
...
- (void)startBackgroundTasks:(NSInteger)time_  target:(id)target_ selector:(SEL)selector_
{
    ...
    //maxmum 180 sec
//    [[UIApplication sharedApplication] setKeepAliveTimeout:BackgroundInterval handler:^{
//        [self initBackgroudTask];
//    }];
    
    [[UIApplication sharedApplication] setKeepAliveTimeout:timerInterval handler:^{
        [self initBackgroudTask];
    }];
...
-(void) initBackgroudTask
{
    dispatch_async(dispatch_get_main_queue(), ^(void) {
        ...
        while([self running])
        {
            [NSThread sleepForTimeInterval:175]; //等待100秒
        }
